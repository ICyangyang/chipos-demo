...
module uart_tx_fifo (
    ...
    localparam ADDR_WIDTH = $clog2(FIFO_DEPTH);  // 地址宽度：自动推导，不可单独覆盖
    ...
    assign tx_fifo_full = (count == 5'd16)/* 你的答案 */;

    // 【TODO-2】tx_fifo_empty：FIFO 空标志
    // 提示：count 等于多少时表示 FIFO 空了？
    assign tx_fifo_empty = (count == 5'd0) /* 你的答案 */;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            //   但 push 应该正常写入
                ...
                data_fifo[ip_count] <= wr_data;
                ip_count <= ip_count + 1;
                count <= count + 5'd1;    // push 生效，pop 被忽略，count+1
